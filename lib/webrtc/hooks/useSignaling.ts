...
export function useSignaling(
    ...
    useEffect(() => {
        ...
        channel
            ...
            .on('broadcast', { event: 'ready' }, (msg) => {
                ...
                if (payload.roomId !== roomId || payload.senderId === currentUserId) return;
                callbacksRef.current.onPeerReady();
            })
            .on('broadcast', { event: 'sdp' }, (msg) => {
                const payload = msg.payload as { type: 'offer' | 'answer'; sdp: string; roomId: string; senderId: string };
                if (payload.roomId !== roomId || payload.senderId === currentUserId) return;
            ...
            .on('broadcast', { event: 'ice' }, (msg) => {
                const payload = msg.payload as { candidate: RTCIceCandidateInit; roomId: string; senderId: string };
                if (payload.roomId !== roomId || payload.senderId === currentUserId) return;
    ...
    const sendIceCandidate = useCallback(async (candidate: RTCIceCandidate) => {
        ...
        await channelRef.current.send({
            type: 'broadcast',
            event: 'ice',
            payload: { candidate: candidate.toJSON(), roomId, senderId: currentUserId },
        });
    }, [roomId, currentUserId]);
