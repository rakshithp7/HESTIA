...
const MATCH_THRESHOLD_START = 0.80;
...
type UseMatchQueueResult = {
    ...
    acceptSuggestedMatch: (overrideMatch?: { queueId: string }, isResponse?: boolean) => Promise<void>;
    rejectSuggestedMatch: () => Promise<void>;
    resetMatchState: () => void;
};

export function useMatchQueue({ currentUserId, config, blockedUserIds, blockedByUserIds }: UseMatchQueueProps): UseMatchQueueResult {
    ...
    // Sync refs
    useEffect(() => { activeQueueIdRef.current = activeQueueId; }, [activeQueueId]);
    useEffect(() => { suggestedMatchRef.current = suggestedMatch; }, [suggestedMatch]);
    ...
    const sendSignal = useCallback((type: 'consent' | 'reject', targetQueueId: string, payload?: Record<string, unknown>) => {
    ...
    const acceptSuggestedMatch = useCallback(async (overrideMatch?: { queueId: string }, isResponse = false) => {
        ...
        try {
            ...
            const isPeerKnownConsented = suggestedMatch?.queueId === target.queueId && suggestedMatch?.peerConsentedToMe;
    ...
    }, [suggestedMatch, currentUserId, activeQueueId, supabase, config.topic, sendSignal]);

    const rejectSuggestedMatch = useCallback(async () => {
        if (!activeQueueId) return;
        try {
            await supabase.from('match_queue').update({ consented_queue_id: null }).eq('id', activeQueueId);
            hasConsentedToQueueIdRef.current = null;
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            ...
            try {
                const excludedIds = Array.from(new Set([...blockedUserIds, ...blockedByUserIds]));

                const elapsedSec = (Date.now() - (startTimeRef.current || Date.now())) / 1000;
                const decay = elapsedSec * MATCH_THRESHOLD_DECAY_RATE;
                const currentThreshold = Math.max(MATCH_THRESHOLD_MIN, MATCH_THRESHOLD_START - decay);
    ...
    }, [status, currentUserId, activeQueueId, config.mode, blockedUserIds, blockedByUserIds, supabase]);
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on(
                ...
                (payload) => {
                    ...
                    const newRow = (payload as any).new as { status: string; room_id: string };
            ...
            .on('broadcast', { event: 'signal' }, (payload) => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                const signal = (payload as any).payload as { type: string; fromQueueId: string; topic?: string };
                if (signal?.type === 'consent') {
                    const isActiveSuggestion = suggestedMatchRef.current?.queueId === signal.fromQueueId;
                    ...
                    if (hasConsentedToQueueIdRef.current === signal.fromQueueId) {
                        ...
                        toast.custom((t) => createElement('div', {
                            ...
                            createElement('div', { key: 'content', className: "flex flex-col gap-1 items-center" }, [
                                createElement('div', { key: 'msg', className: "text-xl font-medium text-foreground" }, `${peerTopic} wants to connect!`),
                                createElement('div', { key: 'desc', className: "text-base text-muted-foreground" }, "They are suggested as a similar match.")
                            ]),
                            createElement('div', { key: 'actions', className: "flex gap-2 justify-center" }, [
                                createElement('button', {
                                    key: 'skip',
                                    className: "inline-flex items-center justify-center rounded-md border border-gray-300 bg-transparent h-11 px-8 text-base font-medium text-gray-400 shadow-sm transition-colors hover:border-gray-300 hover:bg-gray-300 hover:text-gray-700",
                                    onClick: () => {
                                ...
                                createElement('button', {
                                    key: 'conn',
                                    className: "inline-flex items-center justify-center rounded-md bg-primary h-11 px-8 text-base font-medium text-primary-foreground shadow transition-colors hover:bg-primary/90",
                                    onClick: () => { acceptSuggestedMatch({ queueId: signal.fromQueueId }, true); toast.dismiss(t); }
                                }, 'Connect')
                            ]),
                            createElement('div', { key: 'progress-outer', className: "absolute bottom-1 left-4 right-4 h-1" }, [
                                createElement('div', { key: 'progress-track', className: "h-full w-full bg-primary/20 rounded-full overflow-hidden" }, [
                                    createElement('div', { key: 'progress-inner', className: "h-full bg-primary origin-left animate-toast-progress rounded-full" })
                                ])
                            ])
                        ]), { duration: 5000 });
                    }
                    setSuggestedMatch(prev => prev ? { ...prev, peerConsentedToMe: true } : null);
        ...
        if (suggestedMatch?.queueId) {
            suggestedChannel = supabase
                ...
                .on(
                    'postgres_changes',
                    { event: '*', schema: 'public', table: 'match_queue', filter: `id=eq.${suggestedMatch.queueId}` },
                    (payload) => {
    ...
    }, [activeQueueId, status, supabase, suggestedMatch, acceptSuggestedMatch, sendSignal]);
