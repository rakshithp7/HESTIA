...
export async function GET() {
  try {
    ...
    const [{ data: blockedData, error: blockedError }, { data: blockedByData, error: blockedByError }] =
      await Promise.all([
        service.from('blocked_users').select('blocked_user_id').eq('user_id', user.id),
        service.from('blocked_users').select('user_id').eq('blocked_user_id', user.id),
      ]);

    if (blockedError || blockedByError) {
      console.error('[blocked] Failed to load blocked lists', blockedError ?? blockedByError);
      return NextResponse.json({ error: 'Unable to load blocked users' }, { status: 500 });
    }

    const blockedUserIds = blockedData?.map((entry) => entry.blocked_user_id) ?? [];
    const blockedByUserIds = blockedByData?.map((entry) => entry.user_id) ?? [];

    let blockedUsers: { id: string; firstName: string | null; lastName: string | null }[] = [];
    ...
    if (blockedUserIds.length) {
      ...
      if (profilesError) {
        console.error('[blocked] Failed to load blocked user profiles', profilesError);
        return NextResponse.json({ error: 'Unable to load blocked users' }, { status: 500 });
      }
...
export async function DELETE(req: Request) {
  try {
    ...
    const payload = (await req.json().catch(() => null)) as { blockedUserId?: string } | null;
    ...
    if (error) {
      ...
      return NextResponse.json({ error: 'Unable to remove blocked user' }, { status: 500 });
    }
