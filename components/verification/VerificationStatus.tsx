...
export function VerificationStatus({
  ...
  const isRetryable =
    currentStatus === 'requires_input' || currentStatus === 'failed';
      ...
      <div className="max-w-xl text-center space-y-6">
        ...
        <div>
          ...
          <p className="mt-3 text-foreground">
            Thanks for completing identity verification. You can now access the
            Connect experience.
          </p>
          {typeof verifiedAge === 'number' ? (
            <p className="mt-2 text-sm text-foreground">
              Age verified from ID: {verifiedAge}
            </p>
          ) : null}
        </div>
        <div className="flex justify-center gap-4 flex-wrap">
          <Button onClick={() => router.replace('/connect')}>
            Open Connect
          </Button>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
          ...
          <div className="space-y-2 text-center sm:text-left">
            ...
            <p className="text-sm text-foreground">
              You’re all set. Head back to the Connect experience to start
    ...
    <section className="rounded-lg border border-border bg-card/60 p-6 space-y-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1 text-center sm:text-left">
          <p className="text-sm uppercase tracking-wide text-secondary-foreground">
            Current status
          </p>
  ...
  const expectations = (
    <div className="rounded-lg border border-dashed border-border p-6 space-y-3">
      ...
      <ul className="list-disc list-inside text-sm text-foreground space-y-1 pl-4 sm:pl-6 text-left">
        <li>
          Click “Start verification” to launch the secure Stripe Identity flow
          in a new tab.
        </li>
        <li>Have a government-issued ID ready.</li>
        <li>
          We’ll update your status automatically once Stripe completes the
          checks.
        </li>
      </ul>
    </div>
  ...
  const actionButton = (
    <Button
      className="w-full"
      onClick={() => handleStartVerification(isRetryable)}
        ...
        <div className="w-full max-w-3xl space-y-8">
          <header className="text-center space-y-3 px-2">
            ...
            <p className="text-foreground max-w-xl mx-auto text-base sm:text-lg">
              We partner with Stripe to securely confirm you meet our age and
              safety requirements. The process usually takes a few minutes and
              you’ll only need a valid photo ID and your camera.
            </p>
          </header>
