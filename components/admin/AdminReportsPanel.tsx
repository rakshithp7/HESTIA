...
type ReportWithActiveBan = ModerationReport & {
...
const STATUS_FILTERS: (ModerationReportStatus | 'all')[] = [
  'all',
  'pending',
  'resolved',
  'dismissed',
...
export function AdminReportsPanel({
  ...
  const handleOpenReport = React.useCallback(
    async (report: ReportWithActiveBan) => {
      setSelectedReport(report);
      setIsDetailOpen(true);
      setDetailLoading(true);
      setDetail(null);
      setResolutionStatus(
        report.status === 'pending' ? 'resolved' : report.status
      );
      setResolutionNotes('');
      try {
    ...
    async (newStatus: ModerationReportStatus) => {
      ...
      try {
        const response = await fetch(
          `/api/admin/reports/${selectedReport.id}/resolve`,
          {
            method: 'POST',
  ...
  const handleBan = React.useCallback(
    async (durationLabel: BanDurationLabel, customEndsAt?: string) => {
      ...
      try {
        const response = await fetch(
          `/api/admin/reports/${selectedReport.id}/ban`,
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  const handleLiftBan = React.useCallback(
    async (banId: string) => {
      setActionLoading(true);
      try {
        const response = await fetch(`/api/admin/bans/${banId}/lift`, {
          method: 'POST',
        });
        if (!response.ok) {
          const payload = await response.json().catch(() => ({}));
  ...
  const cardWrapperClass =
    variant === 'embedded'
      ? 'space-y-4 rounded-2xl border border-border/60 bg-card/20 p-6 shadow-sm'
      : 'space-y-6';
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-secondary-foreground">
            Review escalations, issue bans, and resolve sessions.
          </p>
        </div>
        <div className="flex items-center gap-3">
          <Select
            value={statusFilter}
            onValueChange={(value) =>
              setStatusFilter(value as ModerationReportStatus | 'all')
            }
          >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {reports.map((report) => (
            <article
              key={report.id}
              className="bg-card/40 rounded-xl border border-border p-4 shadow-sm "
            >
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-secondary-foreground">
                    Reporter: {report.reporter_email ?? report.reporter_id} ·
                    Reported: {report.reported_email ?? report.reported_id}
                  </p>
                </div>
                <div className="flex flex-col items-end gap-2">
                  <StatusBadge status={report.status} />
                  {report.activeBan && (
                    <BanBadge endsAt={report.activeBan.ends_at} />
                  )}
                  <Button
                    size="sm"
              ...
              <section className="space-y-3 rounded-lg border border-border p-4">
                ...
                <div className="mt-2 grid gap-2 text-sm text-secondary-foreground sm:grid-cols-2">
                  ...
                  <p>
                    ...
                    {detail.report.reporter_email ?? detail.report.reporter_id}
                  </p>
                  <p>
                    <strong>Reported:</strong>{' '}
                    {detail.report.reported_email ?? detail.report.reported_id}
                  </p>
                  <p>
                    <strong>Created:</strong>{' '}
                    {formatDateTime(detail.report.created_at)}
                  </p>
                  <p>
                    ...
                    {detail.report.chat_log.map((message) => (
                      <div key={message.id} className="space-y-1">
                        <p className="text-secondary-foreground">
                          <strong>
                            {message.sender === 'me' ? 'Reporter' : 'Peer'}
                          </strong>{' '}
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div className="space-y-2">
                    <Select
                      ...
                      onValueChange={(value) =>
                        setResolutionStatus(value as ModerationReportStatus)
                      }
                    >
                      <SelectTrigger>
                    ...
                    <Button
                      disabled={actionLoading}
                      onClick={() => void handleResolve(resolutionStatus)}
                    >
                      Update status
                    </Button>
                  </div>
                  <div className="space-y-2">
                    <Textarea
                      ...
                      disabled={actionLoading}
                    />
                    <p className="text-right text-xs text-secondary-foreground">
