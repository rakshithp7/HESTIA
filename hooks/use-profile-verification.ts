  ...
  const fetchProfile = useCallback(
    async (id: string, options: { silent?: boolean } = {}) => {
      ...
      try {
        ...
        if (error) {
          console.error('[useProfileVerification] Failed to load profile', error);
          setError('Unable to confirm your verification status. Please refresh.');
        } else {
          setProfile(data);
        }
      } catch (err) {
        console.error('[useProfileVerification] Unexpected profile fetch error', err);
        setError('Unable to confirm your verification status. Please refresh.');
      } finally {
        if (!options.silent) {
          setLoading(false);
  ...
  useEffect(() => {
    ...
    const initialiseProfile = async () => {
      try {
        ...
        if (error) {
          console.error('[useProfileVerification] Failed to resolve user', error);
          setError('Unable to confirm your verification status. Please refresh.');
          setLoading(false);
          return;
        }
        ...
        channel = supabase
          ...
          .subscribe((status) => {
            if (status === 'SUBSCRIBED') {
              console.debug('[useProfileVerification] Subscribed to profile verification updates');
            }
          });
      } catch (err) {
        console.error('[useProfileVerification] Unexpected profile initialisation error', err);
        if (isMounted) {
          setError('Unable to confirm your verification status. Please refresh.');
          setLoading(false);
        }
